...
namespace BADProject.WebUI.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Log the claims associated with the user
                    if (user != null)
                    {
                        var userId = await _signInManager.UserManager.GetUserIdAsync(user);
                        _logger.LogInformation("User ID: {UserId}", userId);

                        var userName = user.UserName; // Directly accessing the UserName property
                        _logger.LogInformation("User Name: {UserName}", userName);

                        foreach (var claim in await _signInManager.UserManager.GetClaimsAsync(user))
                        {
